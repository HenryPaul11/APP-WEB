...
public class ProductoController {
    ...
    @GetMapping("/paginado")
    @Operation(summary = "Listar productos activos con paginacion")
    public Page<Producto> paginado(
            @RequestParam(defaultValue = "0") int page,
            @RequestParam(defaultValue = "10") int size,
            @RequestParam(required = false) String nombre) {
        return service.listarPaginado(nombre, PageRequest.of(page, size));
    }
    ...
    @DeleteMapping("/{id}")
    @Operation(summary = "Eliminar un producto (eliminacion logica); requiere ADMIN")
